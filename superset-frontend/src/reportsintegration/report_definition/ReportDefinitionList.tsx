...
interface ReportDefinitionListProps {
...
function ReportDefinitionList(props: ReportDefinitionListProps) {
  const { addDangerToast, addSuccessToast } = props;

  const {
    state: {
      loading,
      resourceCount: reportDefinitionCount,
    ...
    toggleBulkSelect,
    refreshData,
  } = useListViewResource<ReportDefinition>(
    'report_definitions',
  ...
  const filters: Filters = [
    {
      ...
      fetchSelects: createFetchRelated(
        ...
        createErrorHandler(errMsg =>
          addDangerToast(
            t(
              'An error occurred while fetching report definitions owner values: %s',
              errMsg,
            ),
          ),
        ),
    ...
    },
    {
      ...
      fetchSelects: createFetchRelated(
        ...
        createErrorHandler(errMsg =>
          addDangerToast(
            t(
              'An error occurred while fetching report definitions created by values: %s',
              errMsg,
            ),
          ),
        ),
    ...
    // ...(props.user.userId ? [favoritesFilter] : []),
    {
      ...
      id: 'report_name',
      input: 'search',
      operator: FilterOperator.title_or_name,
    },
  ];
  ...
  if (canCreate) {
    subMenuButtons.push({
      name: (
        <>
          <i className="fa fa-plus" /> {t('Report Definition')}
        </>
      ),
      buttonStyle: 'primary',
      onClick: () => {
        window.location.assign('/report_definitions/add/');
      },
    });
  }
  ...
  return (
    <>
      ...
      <ConfirmStatusChange
        title={t('Please confirm')}
        description={t(
          'Are you sure you want to delete the selected report definitions?',
        )}
        ...
        {confirmDelete => {
          ...
          return (
            <>
              ...
              <ListView<ReportDefinition>
                bulkActions={bulkActions}
                bulkSelectEnabled={bulkSelectEnabled}
                // cardSortSelectOptions={sortTypes}
                className="report-definition-list-view"
                columns={columns}
                count={reportDefinitionCount}
...
export default withToasts(ReportDefinitionList);
