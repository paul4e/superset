...
type PropertiesModalProps = {
  reportEngine: ReportEngineData;
  show: boolean;
  onHide: () => void;
  onSave: (reportDefinition: ReportEngine) => void;
};
...
export default function PropertiesModal({
  ...
  const [name, setName] = useState(reportEngine.verbose_name || '');
  const [description, setDescription] = useState(
    reportEngine.description || '',
  );
  ...
  const fetchReportEngineData = useCallback(
    async function fetchReportEngineData() {
      try {
        const response = await SupersetClient.get({
          endpoint: `/api/v1/report_engines/${reportEngine.report_engine_id}`,
        });
    ...
    [reportEngine.report_engine_id],
  );

  // get the owners of this reportDefition
  useEffect(() => {
    fetchReportEngineData();
  }, [fetchReportEngineData]);

  // update name after it's changed in another modal
  useEffect(() => {
    setName(reportEngine.verbose_name || '');
  }, [reportEngine.verbose_name]);
  ...
  const loadOptions = (input = '') => {
    ...
    return SupersetClient.get({
      endpoint: `/api/v1/report_engines/related/owners?q=${query}`,
    }).then(
      response => {
  ...
  const onSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const updatedReportDefinition = {
        ...res.json.result,
        id: reportEngine.report_engine_id,
      };
  ...
  return (
    <Modal
      show={show}
      onHide={onHide}
      title="Edit Report Engine Properties"
      footer={
        <>
          <Button
