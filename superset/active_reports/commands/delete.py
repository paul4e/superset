...
class DeleteActiveReportCommand(BaseCommand):
    def __init__(self, user: User, active_report_id: int):
        self._actor = user
        self._active_report_id = active_report_id
        self._model: Optional[ActiveReport] = None

    def run(self) -> Model:
        self.validate()
        try:
            active_report = ActiveReportsDAO.delete(self._model)
        except DAODeleteFailedError as ex:
            logger.exception(ex.exception)
            raise ActiveReportDeleteFailedError()
        return active_report

    def validate(self) -> None:
        # Validate/populate model exists
        self._model = ActiveReportsDAO.find_by_id(self._active_report_id)
        if not self._model:
